// Código que esta funcionando 100%
// "use client";
...
// export default function CadastroIndicado({ params }: { params: { email: string } }) {
...
//     usuario_nome: "",
//     usuario_email: decodeURIComponent(params.email),
//     usuario_senha: "",
//     token: token || "",
//   });
...
import { motion, AnimatePresence } from "framer-motion";
import { FaCheckCircle, FaExclamationCircle, FaGift, FaUser, FaLock, FaEnvelope } from "react-icons/fa";
interface CadastroIndicadoProps {
  email: string;
  token: string;
}

export default function CadastroIndicado({ email, token }: CadastroIndicadoProps) {
  const router = useRouter();
  const [form, setForm] = useState({
    usuario_nome: "",
