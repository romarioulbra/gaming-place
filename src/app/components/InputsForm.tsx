...
interface FormularioProps {
 ...
 dadosSelect?: { [key: string]: string }[]; // Ajuste aqui conforme o tipo real
  ...
  fileImage?: (e: React.ChangeEvent<HTMLInputElement>) => void;
}
    ...
    <div className="space-y-6">
      <div>
        ...
        {tipoInput === 'fileInput' && (
          <input
            type="file"
            value={valorInput as string | undefined} // TypeScript precisa do cast pois valor de input file não é string, pode ignorar value se quiser
            accept="image/*"
            onChange={fileImage}
            className="block w-full border rounded text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded file:border-0 file:text-sm file:font-semibold file:bg-rose-500 file:text-white hover:file:bg-rose-600"
            required
