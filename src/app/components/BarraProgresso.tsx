...
export default function BarraProgresso({ shouldRefresh }: { shouldRefresh: boolean }) {
  const [progresso, setProgresso] = useState(0);
  ...
  return (
    <div>
      ...
      <div className="w-full bg-gray-700 rounded-full h-2">
        <div
          className="bg-yellow-400 h-2 rounded-full transition-all duration-500"
          style={{ width: `${Math.min(100, (progresso / 1000) * 100)}%` }}
        ></div>
      </div>
      <p className="text-sm mt-1">Pontuação: {progresso}/{1000}</p>
    </div>
  );
}