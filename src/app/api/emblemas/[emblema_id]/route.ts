...
export async function PUT(req: NextRequest, { params }: { params:  Promise<{ emblema_id: string }> }) {
  try {
    const { emblema_id } = await params;
...
export async function DELETE(req: NextRequest, { params }: { params:  Promise<{ emblema_id: string }> }) {
  try {
    const { emblema_id } = await params;
