...
export default function PerfilDropdown({ 
  ...
  const handleSaveChanges = async () => {
    try {
      // Primeiro verifica se temos um arquivo de imagem selecionado
      if (selectedFile) {
      ...
      // Busca o perfil para obter o ID
      const perfilResponse = await axios.get("/api/perfis");
      const perfilId = perfilResponse.data.perfil_id;
      ...
      // Atualiza a imagem no estado após o upload bem-sucedido
      if (selectedFile) {
        const reader = new FileReader();
        reader.onload = (event) => {
          if (event.target?.result) {
  ...
  return (
    <div className="relative">
      <button onClick={toggleDrawer} className="flex items-center focus:outline-none">
        <Image
          src={formData.imagem.startsWith('blob:') || formData.imagem.startsWith('data:') ? 
               formData.imagem : 
               formData.imagem}
          alt="Avatar"
          width={40}
          height={40}
          className="rounded-full object-cover w-12 h-12 border border-white p-1"
          unoptimized={formData.imagem.startsWith('blob:') || formData.imagem.startsWith('data:')}
          onError={(e) => {
            // Fallback para imagem padrão em caso de erro
            (e.target as HTMLImageElement).src = "/img/avatar.jpg";
          }}
        />
      </button>
