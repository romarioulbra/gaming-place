...
export async function PUT(req: Request, { params }: { params: Promise<{ usuario_id: string }> }) {
  try {
    console.log("Iniciando PUT...");
    console.log("Parâmetros recebidos:", (await params));

    const body = await req.json();
    console.log("Dados recebidos no body:", body);

    // Verifique se o ID é válido
    if (isNaN(Number((await params).usuario_id))) {
    ...
    const usuarioAtualizado = await prisma.usuarios.update({
      where: { usuario_id: Number((await params).usuario_id) },
      data: {
        ...restoDoBody,
