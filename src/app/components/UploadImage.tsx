...
interface UploadImageProps {
  label: string;
  metodoSubmit?: (file: File) => void;
}

export default function UploadImage({ label, metodoSubmit }: UploadImageProps) {
  const [file, setFile] = useState<File | undefined>();
  const [uploadedFileName, setUploadedFileName] = useState<string | null>(null);
  ...
  const handleFileChange = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    if (metodoSubmit) metodoSubmit(selectedFile);
  };
  ...
  return (
    <>
      ...
      <div className="flex flex-col items-center">
        ...
        {uploadedFileName && (
          <div
            className="ml-12 mr-12 mt-4 bg-orange-100 border-l-4 border-green-500 text-green-700 p-4"
            role="alert"
          >
            <p className="font-bold">Sucesso!</p>
            <p>
              A imagem <strong>{uploadedFileName}</strong> foi gravada com sucesso!
            </p>
          </div>
        )}
