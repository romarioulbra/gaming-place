...
interface ModelFormSugestaoProps {
  onClose: () => void;
  dadosUsuario: {
    usuario: {
      id: number | string;
...
// Tipo para os critérios de emblemas
type Criterio = {
  tipo_emblema_id: string;
  tipo_emblema_criterio: string;
};

export function ModelFormSugestao({ onClose, dadosUsuario }: ModelFormSugestaoProps) {
  ...
  // Buscar critérios da API
  useEffect(() => {
    const fetchCriterios = async () => {
      try {
        const response = await axios.get<{ cat_emblemas: Criterio[] }>('/api/categoria_emblemas');
        setCriterios(response.data.cat_emblemas);
      } catch (error) {
        console.error('Erro ao carregar critérios:', error);
      }
  ...
  // Manipulador específico para seleção de critérios
  const handleSelectChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    const selectedOption = criterios.find(crit => crit.tipo_emblema_id === e.target.value);
    setFormData(prev => ({
      ...prev,
    ...
    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50 backdrop-blur-sm transition-opacity duration-300 p-4">
      <div className="relative w-full max-w-4xl bg-white text-gray-800 rounded-xl shadow-2xl overflow-hidden transform transition-all duration-500 scale-95 hover:scale-100 flex flex-col lg:flex-row">
        ...
        <div className="w-full lg:w-1/2 p-6 sm:p-8 bg-gradient-to-br from-white to-indigo-50">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="space-y-1">
              <label htmlFor="sugestao_melhoria_titulo" className="block text-sm font-medium text-gray-700">Assunto *</label>
              <input
                type="text"
                id="sugestao_melhoria_titulo"
                name="sugestao_melhoria_titulo"
            ...
            <div className="flex justify-end space-x-3 pt-4">
              ...
              <button
                type="submit"
                className={`px-4 py-2 text-white rounded-lg focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition-all duration-300 shadow-md hover:shadow-lg ${
