...
export default function AlterarEmblemasCategorias({ dados }: { dados: TipoEmblema }) {
  ...
  useEffect(() => {
    const fetchEmblemas = async () => {
      try {
        ...
        // Se já temos o emblema_id nos dados, não precisamos procurar novamente
  ...
  const handleInputChange = (
    ...
    const newFormData = {
      ...formData,
      [name]: name === "emblema_id" || name === "tipo_emblema_pontos" 
    ...
    // Atualiza o nome do emblema selecionado quando o select muda
    if (name === "emblema_id") {
      const emblemaSelecionado = emblemas.find(e => e.emblema_id === Number(value));
      if (emblemaSelecionado) {
        setSelectedEmblema(emblemaSelecionado.emblema_nome);
      } else {
        setSelectedEmblema("");
      }
    }
            ...
            <div className="flex flex-wrap gap-4">
              <div className="flex-1">
                ...
                >
                  ...
                  <option value="">Atualmente selecionado: <span className="font-medium">{selectedEmblema}</span></option>
                  {emblemas.map((emb) => (
                    <option 
                      key={emb.emblema_id} 
                      value={emb.emblema_id}
                    >
                      {emb.emblema_nome}
                    </option>
                  ))}
